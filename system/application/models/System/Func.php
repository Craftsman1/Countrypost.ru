...
class Func
{
	...
	static public function CorrectCountryAdjective($str){
		...
		$lastLiter		= mb_substr($str,$len-1,1,'UTF-8');
		$preLastLiter	= mb_substr($str,$len-2,1,'UTF-8');
		
		if ($lastLiter == 'я' || $lastLiter == 'и'){
			return mb_substr($str,0,$len-1,'UTF-8').'йский';
		}
		if ($lastLiter == 'а' && $preLastLiter != 'н'){
			return $str.'нский';
		}
		if ($preLastLiter.$lastLiter == 'на'){
			return mb_substr($str,0,$len-1,'UTF-8').'ский';
		}
	...
	/**
	 * Пересобачить в прилагательное в творительном падеже
	 * TODO: дописывать варианты по мере поступления
	 * @param string $str
	 */
	static public function CorrectCountryAdjective2($str)
	{
		$str			= mb_strtolower($str, 'UTF-8');
		$len			= mb_strlen($str, 'UTF-8');
		$lastLiter		= mb_substr($str, $len-1, 1, 'UTF-8');
		...
		if ($lastLiter == 'я')
		{
			if ($preLastLiter == 'и')
			{
				return mb_substr($str, 0, $len-1, 'UTF-8') . 'и';
			}

			if ($preLastLiter == 'е')
			{
				return mb_substr($str, 0, $len-1, 'UTF-8') . 'е';
			}
		}
		else if ($lastLiter == 'й')
		{
