...
class PricelistModel extends BaseModel implements IModel{
	...
	function __construct()
    {
    	...
    	$this->properties->pricelist_price_local	='';
   ...
   /**
	...
	/**
	 * Get local price by weight
	 *
	 * @return array
	 */
	public function getPriceByWeightLocal($weight, $countryFrom, $countryTo, $delivery)
	{
		$result = $this->db->query('
			SELECT MIN(`pricelist`.`pricelist_weight`), `pricelist`.`pricelist_price_local`
			FROM `pricelist`
			WHERE `pricelist`.`pricelist_weight` >= '.floatval($weight).'
				AND `pricelist`.`pricelist_country_from` = '.intval($countryFrom).'
				AND `pricelist`.`pricelist_country_to` = '.intval($countryTo).'
		...
		return ((count($result == 1) &&  $result) ? $result[0]->pricelist_price_local : false);
	}
	
	/**
	 * Конвертирует все цены по текущему курсу
	 *
	 * @return array
	 */
	public function convertAll()
	{
		$this->db->query('
			UPDATE `pricelist`
				INNER JOIN `countries` ON `countries`.`country_id` = `pricelist`.`pricelist_country_from`
				INNER JOIN `currencies` ON `countries`.`country_currency` = `currencies`.`currency_name`
			SET `pricelist`.`pricelist_price` = CEIL(`pricelist`.`pricelist_price_local` / `currencies`.`cbr_cross_rate`)
		');
	}
	...
	public function getPricelist($filter) {
		...
		// выборка
		$result = $this->db->query('
			SELECT `pricelist`.*, `deliveries`.*, `currencies`.`currency_symbol`
			FROM `pricelist`
			INNER JOIN `deliveries` on `deliveries`.`delivery_id` = `pricelist`.`pricelist_delivery`
			INNER JOIN `countries` on `countries`.`country_id` = `pricelist`.`pricelist_country_from`
			INNER JOIN `currencies` on `countries`.`country_currency` = `currencies`.`currency_name`
	...
	/**
	 * Удаление тарифа для пары стран и службы доставки
	 * 
	 * @return (mixed)	- объект pricelist или false в случае ошибки записи в базу
	 */
	public function clear($from, $to, $delivery) 
	{
		return $this->db->delete($this->table, array('pricelist_country_from' => intval($from),
			'pricelist_country_to' => intval($to),
			'pricelist_delivery' => intval($delivery)));
	}
}
?>