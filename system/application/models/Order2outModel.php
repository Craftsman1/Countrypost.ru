...
class Order2outModel extends BaseModel implements IModel{
	...
	function __construct()
    {
    	...
    	$this->properties->order2out_ammount_local	='';
    	$this->properties->order2out_ammount_rur	='';
		...
		$this->properties->order2out_details		= '';
		$this->properties->order2out_payment_service= '';
		$this->properties->order2out_currency		= '';
   ...
   /**
	...
	public function getUserOrders($user_id) {
		return $this->db->query('
			...
			WHERE `order2out_user` = '.intval($user_id).'
				AND `order2out_status` <> "payed"
				AND `order2out_status` <> "deleted"
			ORDER BY `order2out_time` DESC
		')->result();
	}
	
	public function getPayedUserOrders($user_id) {
		return $this->db->query('
			SELECT *
			FROM `'.$this->table.'`
			WHERE `order2out_user` = '.intval($user_id).'
	...
	public function getClientFilteredOrders($filter) {
		
		if (count($filter)) {
			$where = '';			
			foreach ($filter as $key=>$val) {
				if ($key == 'order2out_status' ||
					$key == 'order2out_user' ||
					$key == 'user_login' ||
					$key == 'order2out_id')
				{
		...
		return $this->db->query('
			...
			FROM `'.$this->table.'`
				...
				INNER JOIN `clients` ON `clients`.`client_user` = `users`.`user_id` 
			WHERE '.$where.'
			ORDER BY `order2out_time` ASC
		')->result();
	}
	
	public function getManagerFilteredOrders($filter) {
		...
		return $this->db->query('
			...
			FROM `'.$this->table.'`
				INNER JOIN `users` ON `'.$this->table.'`.`order2out_user` = `users`.`user_id` 
				INNER JOIN `managers` ON `managers`.`manager_user` = `users`.`user_id` 
			WHERE '.$where.'
			ORDER BY `order2out_time` DESC
		')->result();
	}
