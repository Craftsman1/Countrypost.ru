			...
			<form class='admin-inside' id="ordersForm" action="<?=$selfurl?>updatePayedOrdersStatus" method="POST">
				<ul class='tabs'>
					...
					<li class='active'><div><a href='<?=$selfurl?>showPayedOrders'>Оплаченные заказы</a></div></li>
				...
				<div class='table'>
					...
					<table>
						...
						<tr>
							<td nowrap <? if ($order->updated_by_client == 1) : ?>class="red-color" title="Товар изменен клиентом"<? endif; ?>>
							...
							<td align="center">
								...
								<br />Сумма ($) <input type="text" class="payedbox" name="payed<?=$order->order_id?>" value="<?=$order->order_cost_payed?>" />
								...
								<? endif; ?>
								<? if ($order->order_cost > $order->order_cost_payed) : ?>
								<br />Доплатить $<?=$order->order_cost - $order->order_cost_payed?>
								<? endif; ?>
								<br />
