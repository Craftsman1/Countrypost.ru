...
?>
	var snippet = "<td id='odetail_product_name<?=$odetail->odetail_id?>'><?=shortenText($odetail->odetail_product_name, $odetail->odetail_id)?></td><td id='odetail_product_color<?=$odetail->odetail_id?>'><?=shortenText($odetail->odetail_product_color.' / '.$odetail->odetail_product_size.' / '.$odetail->odetail_product_amount, $odetail->odetail_id)?></td><td id='odetail_img<?=$odetail->odetail_id?>'><? if (isset($odetail->odetail_img)) : ?><a href='#' onclick='window.open(\"<?=$odetail->odetail_img?>\");return false;'><?=(strlen($odetail->odetail_img)>17?substr($odetail->odetail_img,0,17).'...':$odetail->odetail_img)?></a><? else : ?><a href='javascript:void(0)' onclick='setRel(<?=$odetail->odetail_id?>);'>Просмотреть скриншот <a rel='lightbox_<?=$odetail->odetail_id?>' href='/client/showScreen/<?=$odetail->odetail_id?>' style='display:none;'>Посмотреть</a></a><? endif; ?></td><td id='odetail_link<?=$odetail->odetail_id?>'><a href='#' onclick='window.open(\"<?=$odetail->odetail_link?>\");return false;'><?=(strlen($odetail->odetail_link)>17?substr($odetail->odetail_link,0,17).'...':$odetail->odetail_link)?></a></td>";
	...
	$('#odetail_product_name<?=$odetail->odetail_id?>,#odetail_product_color<?=$odetail->odetail_id?>,#odetail_img<?=$odetail->odetail_id?>,#odetail_link<?=$odetail->odetail_id?>').remove();
	...
	$('#odetail_pricedelivery<?= $odetail->odetail_id?>').text('<?= $odetail->odetail_pricedelivery?>');
	$('#odetail_price<?= $odetail->odetail_id?>').text('<?= $odetail->odetail_price?>');
	$('#odetail_status<?= $odetail->odetail_id?>').text('<?=$odetail->status_name?>');
	
	<? if (isset($new_order_status)) : ?>
	$('.order_status').text('<?= $new_order_status?>');
	<? endif; ?>
	<? endif; ?>
